...
def distortion_risk_control(x_cal, y_cal, alpha, delta):
    ...
    for lambda_ in tqdm(lambda_candidates):
        ...
        risks.append(empirical_risk+1.645*np.sqrt(sigma_square_lambda)/np.sqrt(len(r_lambdas)))
...
def evaluate_remaining_data(remaining_x_cal, lambda_optimal, remaining_y_cal, beta):
    ...
    #     #         C_lambda_ = (idx, val)
    #     #         selected_human_scores = detoxify_human_scores[idx]
    #     #         detoxify_ft_all.append(detoxify_ft_scores[idx])
    #     #         detoxify_human_all.append(selected_human_scores)
...
def main(n_trials, f1_score, use_dkw, alpha,delta):
    ...
    for key in y_cal:
        ...
        folder_name = f'./results_7/results_key_{key}_trial_{n_trials}_score_{f1_score}_alpha_{alpha}_beta_{delta}'
        if use_dkw: 
